...
def test_active_directory_preset_derives_base_dn_and_search_defaults():
    config = suggest_ldap_config(provider_type="active_directory", domain="adtest.dugganco.local")

    assert config.base_dn == "DC=adtest,DC=dugganco,DC=local"
    ...
    assert "user@adtest.dugganco.local" in config.bind_username_examples
    assert config.group_membership_attribute == "memberOf"


def test_authentik_preset_uses_authentik_ldap_base_and_bind_guidance():
    config = suggest_ldap_config(provider_type="authentik", domain="dugganco.com")

    assert config.base_dn == "dc=ldap,dc=dugganco,dc=com"
    assert config.user_filter == "(objectClass=user)"
    assert config.username_attribute == "cn"
    assert "cn=ldap-bind,ou=users,dc=ldap,dc=dugganco,dc=com" in config.bind_username_examples
